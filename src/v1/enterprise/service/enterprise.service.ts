...
export class EnterpriseService {
  ...
  async updateLogo(id: number, dto: UpdateEnterpriseLogoDto) {
    const enterprise = await this.enterpriseRepository.findOne({
      where: {
        user_id: id,
      },
    ...
    await this.sequelize.transaction(async (transaction: Transaction) => {
      try {
        await enterprise.update({ logo: dto.logo }, { transaction });
        ...
        throw new HttpException(
          'Có lỗi xảy ra không thể cập nhật logo doanh nghiệp lúc này',
          HttpStatus.INTERNAL_SERVER_ERROR,
        );
      }
    });
    await enterprise.reload();
    return sendSuccess({ data: enterprise, msg: 'Cập nhật logo thành công' });
  }
}
